(ns fun-with-quil.animations.xmas-growth
  (:use quil.core))
...
(defn draw []
  (let [[new-x new-y new-r] (random-circle @circles)
        new-d (* 2 new-r)
        reflection-dx (* -0.25 new-d)
        reflection-axis1 (* 0.15 new-d)
        reflection-axis2 (* 2 reflection-axis1)
        c (if (> (random 1) 0.5) [255 0 0] [0 127 0])]
    (push-matrix)
    (apply fill c)
    (translate new-x new-y)
    (ellipse 0 0 new-d new-d)
    (translate reflection-dx reflection-dx)
    (rotate (radians 45))
    (fill 255 255 255)
    (ellipse 0 0 reflection-axis1 reflection-axis2)
    (pop-matrix)

    (if (= (raw-key) \s)
      (save "xmas-growth.png"))
